...
public class DownloadRssFeed extends AsyncTask<String, Void, ArrayList<SingleItem>> {
    ...
    protected void onPostExecute(ArrayList<SingleItem> result) {
        ...
        callerContext.listItems = result;
        callerContext.listViewItems.setAdapter(
                new ArrayAdapter<SingleItem>(callerContext, R.layout.list_layout, result));

        dialog.dismiss();
    }

    public SingleItem dissectItemNode(Element entry){
        try {
            ...
            if (responseCode == HttpURLConnection.HTTP_OK) {
                ...
                if ((itemNodes != null) && (itemNodes.getLength() > 0)) {
                    for (int i = 0; i < itemNodes.getLength(); i++) {
                        newsList.add( dissectItemNode((Element) itemNodes.item(i)) );
                    }
                }
            }
